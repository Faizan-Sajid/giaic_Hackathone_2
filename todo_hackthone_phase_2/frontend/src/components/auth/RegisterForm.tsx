...
// Implementation: Register form component with validation
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (responseData && typeof responseData === 'object' && 'id' in responseData && 'email' in responseData) {
        console.log("Registration successful, setting user in context...");
        ...
        login(userData); // Set user in context immediately (auto-login)
        ...
        setErrors({
          general: 'Registration failed: invalid response from server'
        });
      }
    } catch (error) {
      // Handle API errors
        ...
        switch (error.status) {
          ...
          case HttpStatus.CONFLICT:
            setErrors({
              ...
              general: 'Email already registered. Please login instead.'
            })
            break
    ...
    <div className="w-full max-w-md">
      <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-lg space-y-6">
        ...
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-gray-600">Join TaskFlow to manage your tasks efficiently</p>
        </div>
        ...
        <button
          type="submit"
          disabled={isLoading}
          className="w-full bg-gradient-to-r from-green-600 to-teal-600 text-white py-3 px-4 rounded-lg hover:from-green-700 hover:to-teal-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 transform hover:scale-[1.02]"
        >
          {isLoading ? (
