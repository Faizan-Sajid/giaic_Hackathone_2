...
class APIError(Exception):
    ...
    Spec: Error Handling Strategy (plan.md lines 480-508)
    """
    def __init__(self, status_code: int, detail: str, correlation_id: str = None):
...
class UnauthorizedError(APIError):
    ...
    """
    def __init__(self, detail: str = "Unauthorized", correlation_id: str = None):
        super().__init__(status_code=status.HTTP_401_UNAUTHORIZED, detail=detail, correlation_id=correlation_id)
...
class ForbiddenError(APIError):
    ...
    """
    def __init__(self, detail: str = "Access denied", correlation_id: str = None):
        super().__init__(status_code=status.HTTP_403_FORBIDDEN, detail=detail, correlation_id=correlation_id)
...
class NotFoundError(APIError):
    ...
    """
    def __init__(self, detail: str = "Resource not found", correlation_id: str = None):
        super().__init__(status_code=status.HTTP_404_NOT_FOUND, detail=detail, correlation_id=correlation_id)
...
class ValidationError(APIError):
    ...
    """
    def __init__(self, detail: str = "Invalid input", correlation_id: str = None):
        super().__init__(status_code=status.HTTP_400_BAD_REQUEST, detail=detail, correlation_id=correlation_id)
...
class ConflictError(APIError):
    ...
    def __init__(self, detail: str = "Resource conflict", correlation_id: str = None):
        super().__init__(status_code=status.HTTP_409_CONFLICT, detail=detail, correlation_id=correlation_id)
...
async def api_exception_handler(request: Request, exc: APIError) -> JSONResponse:
    ...
    Spec: Error Handling Strategy (plan.md lines 480-508)
    """
    correlation_id = exc.correlation_id or request.headers.get("X-Correlation-ID", "unknown")

    log_error(
        correlation_id=correlation_id,
        error=exc,
        context=f"API {exc.status_code}: {exc.__class__.__name__}"
    )
